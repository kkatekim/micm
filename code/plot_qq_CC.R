...
install.packages("tidyr", repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
...
create_fisher_and_perm <- function(dt, n_cases, n_controls, variant,
                                   case_col, control_col, pathname,
                                   n_permutations=1000, save_lookup=TRUE, 
                                   lookup_filename='lookup.Rdata',
...
# n_case = 3864
# n_control = 7839
create_all_lookups <- function(filename, n_permutations)
{
  ...
  for (variant in variants)
  {
    ...
    results <- create_fisher_and_perm(cleaned_df, n_cases=3864, n_controls=7839, variant=variant,
...
# need to make this more efficient/parallelize it?
pathname <- here("data")
create_all_lookups(paste0(pathname, "all_protein_variants_fishers.csv"), pathname, 50)
create_all_lookups(paste0(pathname, "afe_protein_variants_fishers.csv"), pathname, 50)